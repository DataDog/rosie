...
public class NestedHtmlElementTest extends E2EBase {
    ...
    public void testHtmlElementNoError() throws Exception {
        Response response = executeTest("bla.js", codeWithError, Language.TYPESCRIPT, ruleCode, "check-aria-element", RULE_TYPE_AST, ENTITY_CHECKED_HTML_ELEMENT, null, true);
        logger.info(response.toString());
        assertEquals(1, response.ruleResponses.size());
        assertEquals(1, response.ruleResponses.get(0).violations.size());

        response = executeTest("bla.js", codeWithoutError, Language.TYPESCRIPT, ruleCode, "check-aria-element", RULE_TYPE_AST, ENTITY_CHECKED_HTML_ELEMENT, null, true);
        logger.info(response.toString());
        assertEquals(1, response.ruleResponses.size());
